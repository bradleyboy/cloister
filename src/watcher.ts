...
export class SessionWatcher {
  ...
  private statusDebounceTimers: Map<string, NodeJS.Timeout> = new Map();
  private pendingStatus: Map<string, "awaiting" | "working" | "idle"> = new Map();

  // Debounce time for status changes to prevent flickering
  ...
  private async handleFileChange(
    ...
    try {
      ...
      // Emit new messages (messages added since last check)
      // For simplicity, emit the last message when file changes
      if (messages.length > 0) {
        ...
        // Check for status change with debouncing to prevent flickering
  ...
  unwatchSession(sessionId: string): void {
    ...
    // Clean up any pending status debounce
